...
class AuthController extends Controller
{
    ...
    public function register(RegisterNewUserRequest $request)
    {
       ...
       $expiration = config('auth.register_cache_expiration', 180);
        Cache::put('user-auth-register-' . $value, compact('code', 'field'),now()->addSeconds($expiration));
    ...
    public static function resendVerificationCodeToUser(ResendVerificationCodeRequest $request)
    {
        ...
        if (empty($user)) {
            ...
            $expiration = config('auth.register_cache_expiration_refresh', 120);
            Cache::put('user-auth-register-' . $value, compact('code', 'field'),now()->addSeconds($expiration));
    ...
    public function sendPasswordResetCode(SendPasswordResetCodeRequest $request)
    {
        ...
        $expiration = config('auth.change_password_cache_expiration', 60);
        Cache::put('user-auth-verification-' . $value, compact('code', 'field'), now()->addSeconds($expiration));
    ...
    public function confirmPasswordResetCode(ResetPasswordRequest $request)
    {
        try {
            DB::beginTransaction();
            $value = $request->getFieldValue();
    ...
    public static function resendPasswordResetCode(ResendPasswordResetCodeRequest $request)
    {
        $field = $request->getFieldName();
        $value = $request->getFieldValue();
        ...
        $code = 123456;
            //random_int(100000, 999999);
        $expiration = config('auth.change_password_cache_expiration_refresh', 120);
        Cache::put('user-auth-verification-' . $value, compact('code', 'field'), now()->addSeconds($expiration));
